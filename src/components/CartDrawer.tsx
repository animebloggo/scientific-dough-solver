...
import { ShoppingCart, Minus, Plus, Trash2, ExternalLink, Loader2, FlaskConical } from "lucide-react";
    ...
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      ...
      <SheetContent className="w-full sm:max-w-lg flex flex-col h-full">
        <SheetHeader className="flex-shrink-0">
          <div className="label-mono mb-1">Specimen Tray</div>
          <SheetTitle className="font-serif text-2xl">Your Cart</SheetTitle>
          <SheetDescription>
            {totalItems === 0 ? "No specimens collected yet." : `${totalItems} item${totalItems !== 1 ? "s" : ""} ready for dispatch`}
          </SheetDescription>
        </SheetHeader>
            ...
            <>
              <div className="flex-1 overflow-y-auto pr-2 min-h-0">
                <div className="space-y-4">
                  {items.map((item) => (
                    <div key={item.variantId} className="flex gap-4 p-3 border border-border rounded-md bg-card">
